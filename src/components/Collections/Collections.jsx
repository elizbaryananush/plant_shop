        ...
        <div className='Collections' id='collections'>
            <div className='carousel'>
                {collectionsDataset.map((e, index) => (
                    <Link to='/bl'>
                        <div>
                            <img className='slider' src={collectionsDataset[index].image} alt="" />
                            <h1>{collectionsDataset[index].name}</h1>
                        </div>
                    </Link>
                ))}
                <svg onClick={() => {
                    document.querySelector('.carousel').scrollLeft -= document.querySelector('.slider').getBoundingClientRect().width + 20;
                }}
